...
export async function registerRoutes(
  ...
  // External game sources API (like Waves)
  app.get("/api/games/sources", async (req, res) => {
    res.json(GAME_SOURCES);
  });

  // Fetch games from external source
  app.get("/api/games/external/:source", async (req, res) => {
    try {
      ...
      res.status(500).json({ error: "Failed to fetch games from source" });
    }
  });

  // Games API (local fallback)
  app.get("/api/games", async (req, res) => {
    try {
      const { category, search, source } = req.query;
      const gameSource = typeof source === "string" ? source : "gnmath";

      let games;
      if (search && typeof search === "string") {
        games = await storage.searchGames(search, gameSource);
      } else if (category && typeof category === "string") {
        games = await storage.getGamesByCategory(category, gameSource);
      } else {
        games = await storage.getGames(gameSource);
      }
