...
export default function Games() {
  ...
  // Fetch games from the selected source using unified API with source parameter
  const { data: games = [], isLoading, refetch } = useQuery<Game[]>({
    queryKey: ["/api/games", { source: gameSource }],
    queryFn: async () => {
      const response = await fetch(`/api/games?source=${gameSource}`);
  ...
  const handleGameClick = (game: Game) => {
    // Use bridge proxy for game URLs, add gameMode to hide URL bar
    const bridgeUrl = `/!!/` + game.url;
    setLocation(`/browser?url=${encodeURIComponent(bridgeUrl)}&gameMode=true&title=${encodeURIComponent(game.title)}`);
  };
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm text-muted-foreground">
            {filteredGames.length} titles from {settings?.gameSource || "PlayerNation"}
          </span>
          <Button
            variant="ghost"
            size="icon"
